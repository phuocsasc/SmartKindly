// server/seedYearTargets.js

import { YearTargetModel } from './src/models/yearTargetModel.js';
        ...
        subFields: [
            {
                ...
                expectedResults: [
                    ...
                    {
                        code: '3',
                        description: 'Thực hiện và phối hợp được các cử động của bàn tay ngón tay, phối hợp tay - mắt',
                        targets: [],
                    },
                ],
            },
            {
                ...
                expectedResults: [
                    {
                        code: '1',
                        description: 'Biết một số món ăn, thực phẩm thông thường và ích lợi của chúng đối với sức khỏe',
                        targets: [],
                    },
                    {
                        code: '2',
...
const seedYearTargets = async () => {
    try {
        ...
        const ageGroups = [
            ...
            { name: 'Khối chồi 4-5 tuổi', data: kindergartenData },
            { name: 'Khối lá 5-6 tuổi', data: kindergartenData },
        ];

        for (const group of ageGroups) {
            // Xóa dữ liệu cũ nếu có
            await YearTargetModel.deleteOne({ ageGroup: group.name });

            // Tạo mới
            const yearTarget = new YearTargetModel({
                ageGroup: group.name,
                mainFields: group.data,
                createdBy: admin._id,
            });

            await yearTarget.save();
            console.log(`✅ Created year target for: ${group.name}`);
        }

        console.log('\n✅ Seeding year targets completed!');
        await CLOSE_DB();
        process.exit(0);
    } catch (error) {
