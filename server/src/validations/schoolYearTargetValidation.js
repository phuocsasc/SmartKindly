...
const targetSchema = Joi.object({
    _id: Joi.alternatives().try(Joi.string(), Joi.object()).optional(), // ✅ Cho phép _id (string hoặc ObjectId)
...
const expectedResultSchema = Joi.object({
    _id: Joi.alternatives().try(Joi.string(), Joi.object()).optional(), // ✅ Cho phép _id
    code: Joi.string().required().messages({
...
const update = async (req, res, next) => {
    const schema = Joi.object({
        ...
        mainFields: Joi.array().items(mainFieldSchema),
    });

    try {
        // ✅ allowUnknown: true để bỏ qua các field không khai báo (như timestamps)
...
export const schoolYearTargetValidation = {
    createNew,
    update,
};
